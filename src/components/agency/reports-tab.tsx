...
export function ReportsTab() {
  ...
  const [shareModal, setShareModal] = useState<null | { title: string; url: string }>(null);
  ...
  // Snapshot the currently-viewed report to brand_reports + return a
  // shareable /r/<token> URL that the agency can paste in an email.
  async function saveSnapshot(kind: string) {
    const title = window.prompt("Title for this snapshot (visible to anyone you share the link with):");
    ...
    try {
      ...
      if (!session?.access_token) { toast("error", "Please sign in again to save reports"); return; }

      // Build the snapshot body. We include just the headline metrics
      // for the brand-report viewer; the full client-side state isn't
      // worth persisting because creators expect numbers, not React.
      const totals = {
        total_pipeline: agencyPipeline.reduce((s: number, d: any) => s + (d.value || 0), 0),
        total_commissions: commissionPayouts.reduce((s: number, p: any) => s + (p.amount || 0), 0),
        roster_size: agencyRoster.length,
        active_campaigns: campaigns.filter((c: any) => c.status === "active").length,
      };
      const body: Record<string, any> = { ...totals };

      if (kind === "creator" || kind === "comparison") {
        body.creators = agencyRoster.slice(0, 20).map((r: any) => ({
          label: r.creator_name || r.name || r.creator_id,
          value: typeof r.totalEarned === "number" ? `$${r.totalEarned.toLocaleString()}` : "—",
        }));
      }
      if (kind === "commission") {
        body.recent_payouts = commissionPayouts.slice(0, 20).map((p: any) => ({
          label: `${p.creator_name || p.creator_id || "Creator"} · ${p.paid_at ? new Date(p.paid_at).toLocaleDateString() : "Pending"}`,
          value: typeof p.amount === "number" ? `$${p.amount.toLocaleString()}` : "—",
        }));
      }
      ...
      const res = await fetch("/api/brand-reports", {
        ...
        body: JSON.stringify({ title: title.trim(), kind, body }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok || !data?.report?.share_token) {
        toast("error", data?.error || "Could not save report");
        return;
      }
      const url = `${window.location.origin}/r/${data.report.share_token}`;
      setShareModal({ title: title.trim(), url });
    } catch {
  ...
  return (
    <div>
      ...
      {!activeReport ? (
        ...
        <div>
          <div className="flex items-center justify-between mb-6">
            <button onClick={() => setActiveReport(null)} className="text-[13px] font-sans font-500 text-[#7BAFC8] hover:underline">
              ← Back to reports
            </button>
            <button
              onClick={() => saveSnapshot(activeReport)}
              disabled={savingReport}
              className="text-[12px] font-sans font-600 px-3 py-1.5 rounded-[8px] bg-[#1A2C38] text-[#FAF8F4] hover:bg-[#0F1E28] disabled:opacity-50"
            >
              {savingReport ? "Saving…" : "Save & share"}
            </button>
          </div>
      ...
      {shareModal && (
        <div className="fixed inset-0 bg-[#0F1E28]/40 z-50 flex items-center justify-center px-4" onClick={() => setShareModal(null)}>
          <div className="bg-white border border-[#D8E8EE] rounded-[12px] p-6 max-w-md w-full" onClick={(e) => e.stopPropagation()}>
            <p className="text-[11px] font-mono uppercase tracking-[0.14em] text-[#8AAABB] mb-2">Snapshot saved</p>
            <h3 className="font-serif text-[24px] text-[#0F1E28] mb-3">{shareModal.title}</h3>
            ...
            <div className="flex gap-2 mb-4">
              <input
                ...
                className="flex-1 h-[40px] rounded-[8px] border border-[#D8E8EE] px-3 text-[12px] font-mono text-[#1A2C38] bg-white"
              ...
              <button
                ...
                className="px-3 h-[40px] rounded-[8px] bg-[#0F1E28] text-white text-[12px] font-sans"
            ...
            <div className="flex justify-end gap-3 items-center">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-[12px] font-sans text-[#3D6E8A] underline underline-offset-2"
              >
                Open preview ↗
              </a>
              <button
                onClick={() => setShareModal(null)}
                className="text-[12px] font-sans text-[#8AAABB] hover:text-[#0F1E28]"
              >
                Close
              </button>
            </div>
