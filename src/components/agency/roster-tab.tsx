                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={async () => {
                    if (inviteEmail) {
                      ...
                      const { data: { session: invSession } } = await sb.auth.getSession();
                      if (!invSession?.access_token) { toast("error", "Please sign in again to send invites"); return; }
                      await fetch("/api/email", {
                        method: "POST",
                        headers: { "Content-Type": "application/json", Authorization: `Bearer ${invSession.access_token}` },
                        body: JSON.stringify({ to: inviteEmail, subject: "You've been invited to join Create Suite", body: "<p>An agency has invited you to connect your Create Suite account. <a href='https://createsuite.co/signup'>Sign up here</a> to get started.</p>" }),
                      });
