...
export async function requireFeature(
  ...
  const { data: profile, error: profileErr } = await sb
    .from("profiles")
    .select("account_type, subscription_status, agency_plan")
    .eq("id", user.id)
    .single();
  ...
  // Check feature gate (passes agency_plan so Growth-only features
  // like API access and SSO correctly reject Starter accounts).
  if (!hasFeatureAccess(profile.account_type, feature, (profile as any).agency_plan)) {
    return {
      ok: false,
      status: 403,
