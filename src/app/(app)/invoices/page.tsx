...
export default function InvoicesPage() {
  ...
  async function sendReminder(inv: Invoice) {
    ...
    try {
      ...
      // /api/email now requires auth for non-admin recipients.
      const { getSupabase } = await import("@/lib/supabase");
      const sb = getSupabase();
      const { data: { session: emailSession } } = await sb.auth.getSession();
      if (!emailSession?.access_token) {
        toast("error", "Please sign in again to send a reminder");
        return;
      }
