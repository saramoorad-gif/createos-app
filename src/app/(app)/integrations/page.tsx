...
function IntegrationsPageContent() {
  ...
  async function handleConnect(oauthType: string) {
    if (!user) return;

    // OAuth state for Google is now an HMAC-signed token issued
    // server-side. Hitting the start endpoint requires a valid
    // session, so an attacker can't craft a state that points at
    ...
    if (oauthType === "google") {
      try {
        ...
        if (!session?.access_token) {
          alert("Please sign in again before connecting an integration.");
          return;
        }
        const res = await fetch(`/api/auth/${oauthType}/start`, {
          method: "POST",
          headers: { Authorization: `Bearer ${session.access_token}` },
        });
        const data = await res.json();
        if (!res.ok || !data?.url) {
          alert(data?.error || "Could not start the OAuth flow. Please try again.");
          return;
        }
        window.location.href = data.url;
      } catch {
        alert("Could not start the OAuth flow. Please try again.");
      }
      return;
    }
    ...
    if (oauthType === "ical") {
      try {
        ...
        if (!res.ok) { alert("Could not fetch calendar."); return; }
        const blob = await res.blob();
        const url = URL.createObjectURL(blob);
        const a = document.createElement("a");
        a.href = url;
        a.download = "createsuite-deals.ics";
        a.click();
        URL.revokeObjectURL(url);
      } catch {
        alert("Could not fetch calendar.");
      }
    }
  }
