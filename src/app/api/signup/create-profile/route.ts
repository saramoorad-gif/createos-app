    ...
    if (insertErr) {
      ...
      if (insertErr.code === "23503") {
        return NextResponse.json(
          { error: "Invalid user ID — auth user not found", gift: giftCode ? { ok: false, reason: "profile_create_failed" } : null },
          { status: 404 }
        );
      }
      ...
      return NextResponse.json(
        { error: "Failed to create profile", detail: insertErr.message, gift: giftCode ? { ok: false, reason: "profile_create_failed" } : null },
        { status: 500 }
      );
    }
...
async function redeemGiftCodeForUser(
  ...
  // Increment uses_count atomically — see the matching comment in
  // /api/gift-codes/redeem. If we lose the CAS, undo the redemption
  // row and surface a soft error.
