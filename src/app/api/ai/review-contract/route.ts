...
export async function POST(req: NextRequest) {
  ...
  // Cap input length so a malicious caller can't run up Anthropic API
  // bills with a multi-megabyte string. ~200K characters is comfortably
  // beyond a real contract but well under any tokenization blowout.
  const MAX_CHARS = 200_000;
  if (typeof contractText !== "string" || contractText.length > MAX_CHARS) {
    return NextResponse.json(
      { error: `Contract text too large (max ${MAX_CHARS.toLocaleString()} characters)` },
      { status: 413 }
    );
  }
