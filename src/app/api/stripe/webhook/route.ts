...
export async function POST(req: NextRequest) {
  ...
  // processed it and can no-op.
  {
    ...
    // Any other error (table missing, RLS, etc.) — log loudly so admins
    // see it, but continue so a broken idempotency table doesn't take
    // down payments. The unique constraint on commissions.stripe_charge_id
    // is still the hard guard for double-credit.
    if (seenErr) {
      console.warn(`[Stripe Webhook] seen-events insert failed: ${seenErr.message} — proceeding without idempotency guard`);
      try {
        await sb.from("error_logs").insert({
          level: "critical",
          source: "api/stripe/webhook/idempotency",
          message: `Idempotency guard failed: ${seenErr.message}`,
          metadata: { eventId: event.id, eventType: event.type, code: seenErr.code },
        });
      } catch {
        // If even error_logs is broken, fall through silently — the
        // webhook itself must still ack so Stripe stops retrying.
      }
    }
  }
  ...
  try {
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        // Map price key to account type. Agency plans also set
        // agency_plan ("starter" or "growth") so feature-gates.ts
        // can correctly gate Growth-only features (RBAC, scheduled
        // reports, API access, SSO).
        let accountType = "free";
        let agencyPlan: "starter" | "growth" | null = null;
        if (priceKey.startsWith("ugc_influencer")) accountType = "ugc_influencer";
        else if (priceKey.startsWith("ugc")) accountType = "ugc";
        else if (priceKey.startsWith("agency_growth")) {
          accountType = "agency";
          agencyPlan = "growth";
        } else if (priceKey.startsWith("agency_starter") || priceKey.startsWith("agency")) {
          accountType = "agency";
          agencyPlan = "starter";
        }
        ...
        // Only write agency_plan when the purchase was an agency tier;
        // a creator-tier upgrade shouldn't blank out a previously-set
        // agency_plan field on edge cases (account_type checks gate
        // agency features anyway).
        if (accountType === "agency") {
          profileUpdate.agency_plan = agencyPlan;
        }
        ...
        // Only after confirming the affiliate exists and is active.
        // Otherwise we'd insert orphan rows that block on the FK and
        // mis-credit deactivated affiliates.
        ...
        if (affiliateId) {
          try {
            const { data: affRow } = await sb
              ...
              .eq("id", affiliateId)
              .single();
            if (!affRow || (affRow as any).status !== "active") {
              console.warn(`[Stripe Webhook] Skipping affiliate referral — affiliate ${affiliateId} not active`);
            } else {
